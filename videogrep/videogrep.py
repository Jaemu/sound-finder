...
def create_supercut(composition, outputfile, padding):
    ...
    videofileclips = dict([(f, VideoFileClip(f, audio=True)) for f in all_filenames])
    ...
    final_clip.to_videofile(outputfile, codec="libx264", fps=24, audio=True)
...
def create_supercut_in_batches(composition, outputfile, padding):
    ...
    clips = [VideoFileClip(filename, audio=True) for filename in batch_comp]
    video = concatenate(clips)
    video.to_videofile(outputfile,  codec="libx264", fps=24, audio=True)
